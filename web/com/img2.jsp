...
function set_img(v,g,w,h,calback){
 $("#"+v).val(g);
 $("#"+v).attr("imgW",w);
 $("#"+v).attr("imgH",h);
 img_hide();
...
function  init_img(fg,gf,calback){
   vg=fg;
   obj=gf;
   $("#"+obj).hide();
  var submitData = {
			  ...
			  var  list=json.list;
			   for (var i = 0; i < list.length; i++) {
				   html+='<div class="pb-15 col-sm-1">'
			          +'<img height="50px" onclick="set_img(\''+fg+'\',\''+list[i].picurl+'\','+list[i].width+','+list[i].height+','+calback+')" onmouseout="selimg_hide()" onmouseover="initSelimg(\'${filehttp}/'+list[i].picurl+'\')" src="${filehttp}/'+list[i].picurl+'"class="border-radius3 width-10 bg-hei-8">'
