			...
			function(json) {
				if (json.state == 0) {
					...
					for(var i=0;i<list.length;i++){
						html+='<div class="section" style="background-image:url(${filehttp}/${bean.picurl});background-size:100% auto;background-color:${bean.backgroundcolor}">';
						
						var spritlist=list[i].spritlist;
						for(var j=0;j<spritlist.length;j++){
							
							var left=parseFloat(spritlist[j].style.marginleft.replace("px",""))*(winWidth/375);
							var top=parseFloat(spritlist[j].style.margintop.replace("px",""))*(winHeight/667);
							console.log(spritlist[j].style.margintop); 
							console.log(top); 
							html+='<a href="'+spritlist[j].url+'"><div'
							    +' style="position:absolute;left:'+left+'px;top:'+top+'px; background-color:'+spritlist[j].style.backgroundcolor+';width:'+spritlist[j].style.width+';height:'+spritlist[j].style.height+';'
							    ...
							    +'style_margin_top="'+spritlist[j].style.margintop+';"><div style="position: absolute;top: 103%;font-size: 16px;width: 100%;text-align: center;color:'+spritlist[j].title_color+'">'+spritlist[j].title+'</div></div>'
							    +'</a>'; 
						}
						html+='</div>';     
					}
