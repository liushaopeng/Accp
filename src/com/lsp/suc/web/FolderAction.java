 ...
 */
@Namespace("/suc")
@Results( { @Result(name ="reload", location = "folder.action",params={"fypage","%{fypage}","type","%{type}"}, type = "redirect") })
public class FolderAction extends TotalAction<ImageInfo>{
	...
	public String execute() throws Exception { 
		...
		List<DBObject> list=baseDao.getList(PubConstants.SUC_FOLDER,whereMap,fypage,10,sortMap,backMap);
		fycount=baseDao.getCount(PubConstants.SUC_FOLDER,whereMap);
	...
	protected void prepareModel() throws Exception {
		...
		try {
			if(_id!=null){
				DBObject db=baseDao.getMessage(PubConstants.SUC_FOLDER, _id);
	...
	public  void   ajaxWeb(){
		...
		List<DBObject>list=baseDao.getList(PubConstants.SUC_FOLDER, whereMap,fypage,1000,sortMap);
		if(list.size()>0){
			sub_map.put("state",0);
	...
	public  void   ajaxAdd(){
		...
		if(StringUtils.isNotEmpty(picurl)){
			io.set_id(mongoSequence.currval(PubConstants.SUC_FOLDER));
			...
			baseDao.insert(PubConstants.SUC_FOLDER, io);
			sub_map.put("state", 0);
		}
		String json = JSONArray.fromObject(sub_map).toString();
		Struts2Utils.renderJson(json.substring(1, json.length() - 1),
	...
	public  void   ajaxDel(){
		...
		if(StringUtils.isNotEmpty(id)){
			...
			int count=baseDao.delete(PubConstants.SUC_FOLDER,whereMap);
			whereMap.clear();
			if(count>0){
				HashMap<String,Object>sortMap=new HashMap<>();
				String title=Struts2Utils.getParameter("sel");
